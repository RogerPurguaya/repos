...
class detalle_simple_fisico_total_d_wizard(models.TransientModel):
	...
	def export_csv(self):
		query= """
				select
				CASE WHEN sl.location_id is not null 
				 THEN CONCAT(sl2.name,'/',sl.name)
				 ELSE sl.name END as almacen,
				 pt.default_code as codigo,
				 pt.name as producto,
				 pc.name as categoria,
				 CASE WHEN pum.name is not null 
				 THEN pum.name ELSE pum2.name END as unidad,
				sum(stock_disponible) as saldo,
				sum(saldo_fisico) as saldo_fisico,
				sum(por_ingresar) as por_ingresar,
				sum(transito) as transito,
				...
				left join stock_location sl on sl.id = ubicacion
				inner join stock_location sl2 on sl2.id = sl.location_id
				left join product_uom pum on pum.id = pt.unidad_kardex
				left join product_uom pum2 on pum2.id = pt.uom_id
				where vst_kardex_onlyfisico_total.date >= 
				'"""+str(self.fiscalyear_id.name)+"""-01-01'
				and vst_kardex_onlyfisico_total.date <= 
				'"""+str(self.fiscalyear_id.name)+"""-12-31'
				group by ubicacion,pum.name,pum2.name,
				pt.default_code,sl.location_id,sl2.name,sl.name,
				product_id,pt.categ_id, pt.name, pc.name
				order by ubicacion,product_id, pt.categ_id
		"""
		if len(self.env['main.parameter'].search([]))==0:
			raise exceptions.Warning('No se han encontrado los parametros de configuracion')
		direccion = self.env['main.parameter'].search([])[0].dir_create_file
		self.env.cr.execute("""
				copy ("""+query+""") to
				'"""+direccion+u"""saldos.csv' WITH DELIMITER ',' CSV HEADER
				""")
		f = open(direccion + 'saldos.csv', 'rb')
