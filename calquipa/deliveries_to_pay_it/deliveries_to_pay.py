...
class account_move(models.Model):
	...
	def create(self, vals):
		...
		if 'rendicion_id' in vals:
			...
			x.rendicion_id.write({'done_move': [(3, x.id)]})
		return x
...
class deliveries_to_pay(models.Model):
	...
	def export_excel(self):
		...
		worksheet.write(7,7, "Estado",boldbord)
		...
		worksheet1.write(7,7, "Glosa",boldbord)
		worksheet1.write(7,8, "Estado",boldbord)
		...
		for line in self.deliver_move:
			...
			worksheet.write(x,6,line.amount_deliverx ,numberdos)
		...
		for line in self.done_move:
			...
			worksheet1.write(y,6,line.amount_deliverx ,numberdos)
			#worksheet1.write(x,7,line.amount_currency_deliver ,numberdos)
			worksheet1.write(y,7,line.glosa_deliver if line.glosa_deliver else '',bord)
	...
	def _done_amount(self):
		...
		for deliver in self:
			...
			for done_move in done_moves:
				for line in done_move.line_id:
					if line.account_id.id == parameter.deliver_account_me.id :
						val += float(line.amount_currency)
					if  line.account_id.id == parameter.deliver_account_mn.id:
						val += float(line.debit- line.credit)
					
			deliver.done_amount = abs(val)
	...
	deliver_a = fields.Char('Deliver A', size=50)
	asiento_d_a = fields.Many2one('account.move','Asiento D A')
	deliver_b = fields.Char('Deliver B', size=50)
	asiento_d_b = fields.Many2one('account.move','Asiento D B')
	done_a = fields.Char('Done A', size=50)
	asiento_done_a = fields.Many2one('account.move','Asiento DONE A')
	done_b = fields.Char('Done B', size=50)
	asiento_done_b = fields.Many2one('account.move','Asiento DONE B')
	done_loan = fields.Char('Done Loan', size=50)
	asiento_d_loan = fields.Many2one('account.move','Asiento D LOAN')
...
class print_entregas_rendir_pdf(models.Model):
	...
	def cabezera(self,c,wReal,hReal):
		...
		if self.seleccionar== '1':
			...
			data= [[ paragraph1 ,paragraph2,paragraph3,paragraph4,paragraph5,paragraph6,paragraph7]]

			t=Table(data,colWidths=(50,70,50,50,90,100,75), rowHeights=(15))
		...
		if self.seleccionar== '2':
			...
			data= [[ paragraph1 ,paragraph2,paragraph3,paragraph4,paragraph5,paragraph6,paragraph7 ,paragraph9]]

			t=Table(data,colWidths=(50,66,46,46,71,91,61,91), rowHeights=(15))
	...
	def reporteador(self):
		...
		if self.seleccionar== '1':
			...
			for line in obj_entrega.deliver_move:
				...
				c.drawRightString( 492 ,pos_inicial, '{:,.2f}'.format(decimal.Decimal ("%0.2f"%line.amount_deliverx )))
		...
		if self.seleccionar== '2':
			...
			for line in obj_entrega.done_move:
				...
				c.drawRightString( 438 ,pos_inicial, '{:,.2f}'.format(decimal.Decimal ("%0.2f"%line.amount_deliverx )))
