...
class account_bank_report(models.Model):
	...
	def compute_documento(self):
		voucher = self.env['account.voucher'].search([('move_id','=',self.nro_asiento.id)])
		if voucher and len(voucher)>0 and voucher[0]:
			flag = 0
			compro = None
			for i in voucher[0].line_dr_ids:
				if compro==None:
					compro = i.nro_comprobante
				elif compro== i.nro_comprobante:
					pass
				else:
					flag =1

			for i in voucher[0].line_cr_ids:
				if compro==None:
					compro = i.nro_comprobante
				elif compro== i.nro_comprobante:
					pass
