...
instance.web.form.CompletionFieldMixin = {
    ...
    get_search_result: function(search_val) {
        ...
        if (this.field.type == "many2one")
        {
            if (this.field.relation == "account.account" || this.field.relation == 'account.analytic.account')
            {
                if (this.view.model == "account.invoice" || this.view.model == "account.invoice.line" || this.view.model == "account.move" || this.view.model == "account.move.line" || this.view.model == "product.product" || this.view.model == "product.template" || this.view.model == "product.category")
                {
                    flag_v = 2;
                }
            }    
        ...
        return this.orderer.add(dataset.name_search(
                search_val, new instance.web.CompoundDomain(toop, exclusion_domain),
...
instance.web.form.FieldMany2One = instance.web.form.AbstractField.extend(instance.web.form.CompletionFieldMixin, instance.web.form.ReinitializeFieldMixin, {
    ...
    init: function(field_manager, node) {
        ...
        if (this.field.type == 'many2one')
        {
            ...
            if (this.field.relation == 'purchase.requisition')
            {
                if (this.view.model == 'purchase.order')
                {
                    flag_f = false;
                }
            }   
...
*/
        if (this.session.uid == 1 || this.session.uid == 17 || this.session.uid == 36) 
        {
            flag_f = false;
        }
        if (flag_f == true)
