...
class exchange_diff_line(osv.osv):
	...
	def make_process_records(self, cr, uid, context):
		...
		if tipo == 'payable':
			...
			if len(moves_ids) > 0:
				...
				for line in self.pool.get('account.move.line').browse(cr, uid, moves_ids, context):
					...
					if index in fact:
						...
						fact[index]['amount'] = fact[index]['amount_currency'] * fact[index]['exchange']
				...
				for key, value in fact.items():
					...
					value['amount'] = -1 * (value['amount_currency'] * value['exchange'])
			...
			exc = open('verificar_exchanfe_diff.txt','w')
