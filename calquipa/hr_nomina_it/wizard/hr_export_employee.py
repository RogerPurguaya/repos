...
class hr_export_employee(models.TransientModel):
	...
	def export_employee(self):
		...
		worksheet.write(2,20,'DIST. CC',boldbord)
		worksheet.write(2,21,'EPS',boldbord)
		worksheet.write(2,22,'AFILIACIO',boldbord)
		worksheet.write(2,23,'COMISION MIXTA',boldbord)
		worksheet.write(2,24,'FECHA DE AFILIACION',boldbord)
		worksheet.write(2,25,'CUSSPP',boldbord)
		worksheet.write(2,26,'BANCO CTS',boldbord)
		worksheet.write(2,27,'Nro CTA CTS',boldbord)
		worksheet.write(2,28,'BANCO REM',boldbord)
		worksheet.write(2,29,'Nro CTA REM',boldbord)
		worksheet.write(2,30,'FIN CONTRATO',boldbord)
		worksheet.write(2,31,u'CORREO ELECTRÓNICO',boldbord)
		...
		for empleado in self.env['hr.employee'].search([],order ='last_name_father,last_name_mother,first_name_complete'):
			...
			worksheet.write(n,20,empleado.dist_c.codigo if empleado.dist_c.codigo else '',normal)
			worksheet.write(n,21,'SI' if empleado.use_eps else 'NO',normal)
			worksheet.write(n,22,empleado.afiliacion.name,normal)
			worksheet.write(n,23,'SI'  if empleado.c_mixta else 'NO',normal)
			worksheet.write(n,24,empleado.fecha_afiliacion,normal)
			worksheet.write(n,25,empleado.cusspp if empleado.cusspp else '',normal)
			worksheet.write(n,26,empleado.banco_cts if empleado.banco_cts else '',normal)
			worksheet.write(n,27,empleado.cta_cts if empleado.cta_cts else '',normal)
			worksheet.write(n,28,empleado.banco_rem if empleado.banco_rem else '',normal)
			worksheet.write(n,29,empleado.cta_rem if empleado.cta_rem else '',normal)	
			worksheet.write(n,30,empleado.end_contract_date if empleado.end_contract_date else '',normal)
			worksheet.write(n,31,empleado.work_email if empleado.work_email else '',normal)
			n=n+1
