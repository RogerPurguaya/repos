...
where periodo_num(api.code) >= periodo_num('""" + str(self.periodo_ini.code) + """') and periodo_num(api.code) <= periodo_num('""" + str(self.periodo_fin.code) + """')
and am.state != 'draft'
...
order by code,empresa
		...
		if self.mostrar == 'excel':
			...
			worksheet.write(0,0, u"Análisis de Saldos x Periodo", bold)
