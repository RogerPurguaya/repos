...
class cts_wizard(models.TransientModel):
	...
	def do_rebuild(self):
		...
		if self.forma == '1':
			return self.get_pdf(hr.cts_lines2.ids, self.digital_sgn)			
		if self.forma == '2':
			if len(hrl) == 0:
				raise osv.except_osv("Alerta!", u"No existe el empleado en el tareo.")
			else:
				return self.get_pdf(hrl.id, self.digital_sgn)
		return True
	...
	def get_pdf(self, htl_id, digital_sgn):
		...
		IDGS = False
		if digital_sgn:
			fim = open(direccion+'tmpctsr.png','wb')
			fim.write(digital_sgn.decode('base64'))
			fim.close()
			IDGS = Image(direccion+'tmpctsr.png')
			IDGS.drawHeight = 40
			IDGS.drawWidth = 120
		...
		#------------------------------------------------------Insertando Data-----------------------------------------
		lines = self.env['hr.cts.line'].search([('cts','=',self.env.context['active_id']),('id','in',htl_id)])
		for line in lines:
			...
			text = u"""<b>CALQUIPA S.A.C.</b> con RUC <b>Nº 20455959543</b>, domiciliada en Urbanización San José D-11 Yanahuara, 
								representado por su <b>"""+self.in_charge.job_id.name+(u' Señor ' if self.in_charge.gender == 'male' else u' Señora ')+self.in_charge.name_related+u"""</b>, 
			...
			table2=Table(datat,colWidths=[350,20,80],rowHeights=15)
			...
			if employee.currency.name == 'USD':
				...
				table3=Table(datat,colWidths=[350,20,80],rowHeights=15)
			...
			dataf=[
			[IDGS if IDGS else '','',''],
			['CALQUIPA S.A.C.','',employee_name],
			['EMPLEADOR','','TRABAJADOR(A)'],
			]
			table4=Table(dataf,colWidths=[200,50,200], rowHeights=15)
