...
class small_cash_view(models.Model):
	...
	def init(self,cr):
		...
		cr.execute(""" DROP VIEW IF EXISTS small_cash_view;
...
from 
	...
	from 
		(select concat(a1.partner_id,a1.account_id,a1.type_document_id,nro_comprobante) as identifica,a1.id,move_id,partner_id,type_document_id,account_id,nro_comprobante,credit as monto from account_move_line as a1
		left join account_account a2 on a2.id=a1.account_id 
		where a2.type='payable' and debit =0 and credit > 0) as provision
		inner join (
		select concat(x3.partner_id,x3.account_id,x3.type_document_id,x3.nro_comprobante) as identifica,x4.small_cash_id,x3.move_id,x3.partner_id,x3.type_document_id,x3.account_id, x3.nro_comprobante
		--x3.debit 
		from (select x1.id,x1.move_id,x1.partner_id,x1.type_document_id,x1.account_id,x1.nro_comprobante 
		      --,x1.debit
		      from account_move_line as x1
		left join account_account x2 on x2.id=x1.account_id
		where x2.type='payable') as x3
...
left join account_period z5 on z5.id=z1.period_id
left join it_type_document z6 on z6.id=tt.type_document_id
