...
class hr_vacation_detalle(models.Model):
	_name = 'hr.vacation.detalle'

	employee_id = fields.Many2one('hr.employee', u'Empleado', required = True)
	dni         = fields.Char(u'Dni', required = True)
	codigo_trab = fields.Char(u'Código trabajador', required = True)
	period_id   = fields.Many2one('account.period',u'Periodo', required = True)
	inicio      = fields.Date(u'Inicio', required = True)
	fin         = fields.Date(u'Fin', required = True)
	dias        = fields.Integer(u'Días', required = True)
	observacion = fields.Text(u'Observaciones')

	@api.model
	def create(self, vals):
		ap  = self.env['account.period'].search([('id','=',vals['period_id'])])[0]
		hva = self.env['hr.vacation.anhos'].search([('fiscalyear_id.code','=',ap.fiscalyear_id.code)])
		if not len(hva):
			raise osv.except_osv('Alerta!',u"No existe el año "+ap.fiscalyear_id.code+u" creado en el menú 'Años'")
		return super(hr_vacation_detalle,self).create(vals)
	...
	def write(self, vals):
		if 'period_id' in vals:
			...
			if not len(hva):
				raise osv.except_osv('Alerta!',u"No existe el año "+ap.fiscalyear_id.code+u" creado en el menú 'Años'")
		return super(hr_vacation_detalle,self).write(vals)

	@api.onchange('employee_id')
	def onchange_employee_id(self):
		if self.employee_id.id:
			self.dni = self.employee_id.identification_id
			self.codigo_trab = self.employee_id.codigo_trabajador
		else:
			self.dni = False

	@api.onchange('inicio', 'fin')
	def onchange_inicio_fin(self):
		if self.inicio and self.fin:
			f = datetime.strptime(str(self.fin), '%Y-%m-%d')
			i = datetime.strptime(str(self.inicio), '%Y-%m-%d')
			res = f - i + timedelta(days=1)
			self.dias = res.days	

class hr_vacation_anhos(models.Model):
	_name     = 'hr.vacation.anhos'
	_rec_name = 'fiscalyear_id'
	_order    = 'fiscalyear_id'

	fiscalyear_id = fields.Many2one('account.fiscalyear', u'Año Fiscal', required=True)
	nro_trab      = fields.Integer(u'Número de Trabajadores', compute="compute_nro_trab")
	normal		  = fields.Integer(u'Días normal')

	vacation_lines = fields.One2many('hr.vacation.anhos.lines','vacation_id', 'lineas')

	@api.model
	def create(self, vals):
		af  = self.env['account.fiscalyear'].search([('id','=',vals['fiscalyear_id'])])	
		hva = self.env['hr.vacation.anhos'].search([('fiscalyear_id','=',vals['fiscalyear_id'])])
		if len(hva) >= 1:
			raise osv.except_osv('Alerta!',u"ya existe el año "+af.code)
		return super(hr_vacation_anhos,self).create(vals)

	@api.one
	def write(self, vals):
		if 'fiscalyear_id' in vals:
			af  = self.env['account.fiscalyear'].search([('id','=',vals['fiscalyear_id'])])	
			hva = self.env['hr.vacation.anhos'].search([('fiscalyear_id','=',vals['fiscalyear_id'])])
			if len(hva) >= 1:
				raise osv.except_osv('Alerta!',u"ya existe el año "+af.code)
			for i in self.vacation_lines:
				i.unlink()
		return super(hr_vacation_anhos,self).write(vals)

	@api.one
	def unlink(self):
		for i in self.vacation_lines:
			i.unlink()
		return super(hr_vacation_anhos,self).unlink()

	@api.one
	def compute_nro_trab(self):
		self.nro_trab = len(self.vacation_lines)

	@api.one
	def actualizar(self):
		he = self.env['hr.employee'].search([])
		for employee in he:
			emp_anho = int(employee.fecha_ingreso.split("-")[0])
			fis_anho = int(self.fiscalyear_id.code)
			if emp_anho <= fis_anho:
				empleados_lineas = [line.employee_id.id for line in self.vacation_lines]
				vals = {}
				vals['fiscalyear_id'] = self.fiscalyear_id.id
				vals['dni']           = employee.identification_id
				vals['codigo_trab']   = employee.codigo_trabajador
				vals['employee_id']   = employee.id
				vals['fecha_ingreso'] = employee.fecha_ingreso

				hva = self.env['hr.vacation.anhos'].search([('fiscalyear_id.code','=',str(int(self.fiscalyear_id.code)-1))])
				if len(hva):
					hval = self.env['hr.vacation.anhos.lines'].search([('employee_id','=',vals['employee_id']),('vacation_id','=',hva.id)])
					if len(hval):
						hval = hval[0]
						vals['fecha_computo'] = datetime.strptime(hval.fecha_vacaciones,"%Y-%m-%d")
					else:
						comp_f = datetime.strptime(employee.fecha_ingreso,"%Y-%m-%d").replace(year=int(self.fiscalyear_id.code))
						vals['fecha_computo'] = comp_f
				else:
					comp_f = datetime.strptime(employee.fecha_ingreso,"%Y-%m-%d").replace(year=int(self.fiscalyear_id.code))
					vals['fecha_computo'] = comp_f

				f_comp  = vals['fecha_computo']
				f_comp  = f_comp + relativedelta.relativedelta(days=-(f_comp.day-1))
				f_comp2 = vals['fecha_computo'] + relativedelta.relativedelta(years=1)
				f_comp2 = f_comp2 + relativedelta.relativedelta(days=(monthrange(f_comp2.year,f_comp2.month)[1]-f_comp2.day))
				ap = self.env['account.period'].search([('date_start','>=',f_comp),('date_stop','<=',f_comp2)])
				ht = self.env['hr.tareo'].search([('periodo','in',ap.ids)])
				faltas = 0
				for tareo in ht:
					htl = self.env['hr.tareo.line'].search([('employee_id','=',vals['employee_id']),('tareo_id','=',tareo.id)])
					faltas += htl[0].dias_suspension_perfecta if len(htl) else 0
				vals['dias_no_lab'] = faltas

				vals['fecha_vacaciones'] = vals['fecha_computo'] + relativedelta.relativedelta(days=vals['dias_no_lab'], years=1)

				vac_f = datetime.strftime(vals['fecha_vacaciones'],"%Y-%m-%d").split("-")
				ap    = self.env['account.period'].search([('code','=',vac_f[1]+"/"+vac_f[0])])
				vals['period_id'] = ap[0].id if len(ap) else False

				hvd = self.env['hr.vacation.detalle'].search([('period_id','=',vals['period_id']),('employee_id','=',vals['employee_id'])])
				res = 0
				for vac in hvd:
					res += vac.dias
				vals['dias_gozados'] = res

				vals['dias_pendientes'] = self.normal - vals['dias_gozados']

				vac_date = vals['fecha_vacaciones']
				now      = datetime.now()
				if vals['dias_pendientes'] == 0:
					vals['control'] = "1"
				elif abs(vac_date.year-now.year) >= 0 and abs(vac_date.year-now.year) < 2:
					vals['control'] = "2"
				elif abs(vac_date.year-now.year) >= 2:
					vals['control'] = "3"
				else:
					vals['control'] = "4"

				if employee.id not in empleados_lineas:
					vals['vacation_id'] = self.id
					n_hval = self.env['hr.vacation.anhos.lines'].create(vals)
				else:
					self.env['hr.vacation.anhos.lines'].search([('employee_id','=',employee.id),('vacation_id','=',self.id)])[0].write(vals)

class hr_vacation_anhos_lines(models.Model):
	_name = 'hr.vacation.anhos.lines'

	vacation_id = fields.Many2one('hr.vacation.anhos', 'padre')
	...
	codigo_trab 	 = fields.Char(u'Código trabajador')
	fecha_ingreso    = fields.Date('Fecha ingreso')
	fecha_computo    = fields.Date('Fecha computo')
	dias_no_lab      = fields.Integer(u'Días no laborados')
	fecha_vacaciones = fields.Date(u'Fecha vacaciones')
	period_id        = fields.Many2one('account.period', u'Periodo vacacional')
	dias_gozados     = fields.Integer(u'Días gozados')
	dias_pendientes  = fields.Integer(u'Días pendientes')
	control          = fields.Selection([("1",u"Vacaciones gozadas."),
										 ("2",u"Vacaciones pendientes."),
										 ("3",u"Limite de vacaciones (Indemnización)."),
										 ("4",u"Otro")],u'Control')