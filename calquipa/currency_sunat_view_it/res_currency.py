...
class res_currency_rate(models.Model):
	...
	def write(self,vals):
		...
		date_tmp = "Indefinido"
		if 'date_sunat' in vals:
			date_tmp = str(vals['date_sunat'])[:7].replace("-","/")

		if self.date_sunat:
			date_tmp = str(self.date_sunat)[:7].replace("-","/")
	...
	def create(self,vals):
		...
		if t.date_sunat:
			t.period_name = str(t.date_sunat)[:7].replace("-","/")
		else:
...
class res_currency_wizard_optional(models.Model):
	...
	def do_manual(self):
		...
		tmp_fn = self.env['res.currency.rate'].search([('currency_id','=',currency_extra.id),('date_sunat','=',str(self.fecha_unica))])
