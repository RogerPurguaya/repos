...
class grupo_report_intercompanias(models.Model):
	_name = 'grupo.report.intercompanias'
...
class tipo_report_intercompanias(models.Model):
	_name = 'tipo.report.intercompanias'
...
class rm_report_intercompanias_line(models.Model):
	_name = 'rm.report.intercompanias.line'


	rm_report_intercompanias_id = fields.Many2one('rm.report.intercompanias','Cabezera')

		

	cuenta = fields.Char('Cuenta',required=False)
	tipo = fields.Many2one('tipo.report.intercompanias','Tipo',required=True)
	concepto = fields.Char('Concepto',required=True)
	grupo = fields.Many2one('grupo.report.intercompanias','Grupo',required=True)
	...
	@api.one
	def get_monto(self):
		m = str(self.rm_report_intercompanias_id.period_actual.code).split('/')
		m = int(m[0])
		mont = 0
		cant = {
			1: self.enero,
		...
		mont = cant[int(self.rm_report_intercompanias_id.period_actual.code.split('/')[0])]
		self.monto = mont
	...
	def get_acumulado_pciento(self):
		if self.acumulado != 0:
			self.acumulado_pciento = self.acumulado / self.rm_report_intercompanias_id.total_general
		else:
	...
	def get_promedio_pciento(self):
		if self.acumulado != 0:
			self.promedio_pciento = self.promedio / self.rm_report_intercompanias_id.total_promedio_general
		else:
...
class rm_report_intercompanias(models.Model):
	_name= 'rm.report.intercompanias'

	fiscal = fields.Many2one('account.fiscalyear','Año Fiscal',required=True)
	sitio = fields.Char('Sitio',required=False)
	centro_de_costo = fields.Char('Centro de Costo',readonly=True,default='Intercompañias')
	...
	conf_line_ids = fields.One2many('rm.report.intercompanias.line','rm_report_intercompanias_id','Lineas de Configuración')
	...
	def unlink(self):
		...
		return super(rm_report_intercompanias,self).unlink()

	@api.one
	def copy(self,default):
		t= super(rm_report_intercompanias,self).copy(default)
		for i in self.conf_line_ids:
			vals_i = {
				'rm_report_intercompanias_id':t.id,
				'cuenta':i.cuenta,
				'tipo':i.tipo.id,
				'concepto':i.concepto,
				'grupo':i.grupo.id,
			...
			self.env['rm.report.intercompanias.line'].create(vals_i)
		return t
	...
	def create(self,vals):
		...
		return super(rm_report_intercompanias,self).create(vals)
	...
	def calculate(self):
		...
		self.env.cr.execute("""
update rm_report_intercompanias_line opt set
enero= 0, febrero = 0,
marzo= 0, abril= 0,
mayo= 0, junio= 0,
julio= 0, agosto= 0,
...
where opt.rm_report_intercompanias_id = """ + str(self.id) + """
""")
...
where opt.cuenta = saldos.nivel1 and opt.rm_report_intercompanias_id = """ + str(self.id) + """
			""")
	...
	def export_excel(self):
		...
		workbook = Workbook(direccion +'Reporte_Intercompañias.xlsx')
		...
		f = open(direccion + 'Reporte_Intercompañias.xlsx', 'rb')
		
		vals = {
			'output_name': 'Reportes_Mexicanos_Intercompañias.xlsx',
