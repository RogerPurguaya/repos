...
class grupo_report_capacitacion(models.Model):
	_name = 'grupo.report.capacitacion'
...
class tipo_report_capacitacion(models.Model):
	_name = 'tipo.report.capacitacion'
...
class rm_report_capacitacion_line(models.Model):
	_name = 'rm.report.capacitacion.line'


	rm_report_capacitacion_id = fields.Many2one('rm.report.capacitacion','Cabezera')

		

	cuenta = fields.Char('Cuenta',required=False)
	tipo = fields.Many2one('tipo.report.capacitacion','Tipo',required=True)
	concepto = fields.Char('Concepto',required=True)
	grupo = fields.Many2one('grupo.report.capacitacion','Grupo',required=True)
	...
	@api.one
	def get_monto(self):
		m = str(self.rm_report_capacitacion_id.period_actual.code).split('/')
		m = int(m[0])
		mont = 0
		cant = {
			1: self.enero,
		...
		mont = cant[int(self.rm_report_capacitacion_id.period_actual.code.split('/')[0])]
		self.monto = mont
	...
	def get_acumulado_pciento(self):
		if self.acumulado != 0:
			self.acumulado_pciento = self.acumulado / self.rm_report_capacitacion_id.total_general
		else:
	...
	def get_promedio_pciento(self):
		if self.acumulado != 0:
			self.promedio_pciento = self.promedio / self.rm_report_capacitacion_id.total_promedio_general
		else:
...
class rm_report_capacitacion(models.Model):
	_name= 'rm.report.capacitacion'

	fiscal = fields.Many2one('account.fiscalyear','Año Fiscal',required=True)
	sitio = fields.Char('Sitio',required=False)
	centro_de_costo = fields.Char('Centro de Costo',readonly=True,default='Capacitación')
	...
	conf_line_ids = fields.One2many('rm.report.capacitacion.line','rm_report_capacitacion_id','Lineas de Configuración')
	...
	def unlink(self):
		...
		return super(rm_report_capacitacion,self).unlink()

	@api.one
	def copy(self,default):
		t= super(rm_report_capacitacion,self).copy(default)
		for i in self.conf_line_ids:
			vals_i = {
				'rm_report_capacitacion_id':t.id,
				'cuenta':i.cuenta,
				'tipo':i.tipo.id,
				'concepto':i.concepto,
				'grupo':i.grupo.id,
			...
			self.env['rm.report.capacitacion.line'].create(vals_i)
		return t
	...
	def create(self,vals):
		...
		return super(rm_report_capacitacion,self).create(vals)
	...
	def calculate(self):
		...
		self.env.cr.execute("""
update rm_report_capacitacion_line opt set
enero= 0, febrero = 0,
marzo= 0, abril= 0,
mayo= 0, junio= 0,
julio= 0, agosto= 0,
...
where opt.rm_report_capacitacion_id = """ + str(self.id) + """
""")
...
where opt.cuenta = saldos.nivel1 and opt.rm_report_capacitacion_id = """ + str(self.id) + """
			""")
	...
	def export_excel(self):
		...
		workbook = Workbook(direccion +'Reporte_Capacitación.xlsx')
		...
		f = open(direccion + 'Reporte_Capacitación.xlsx', 'rb')
		
		vals = {
			'output_name': 'Reportes_Mexicanos_Capacitación.xlsx',
