...
class grupo_report_micronizado(models.Model):
	_name = 'grupo.report.micronizado'
...
class tipo_report_micronizado(models.Model):
	_name = 'tipo.report.micronizado'
...
class rm_report_micronizado_line(models.Model):
	_name = 'rm.report.micronizado.line'


	rm_report_micronizado_id = fields.Many2one('rm.report.micronizado','Cabezera')

		

	cuenta = fields.Char('Cuenta',required=False)
	tipo = fields.Many2one('tipo.report.micronizado','Tipo',required=True)
	concepto = fields.Char('Concepto',required=True)
	grupo = fields.Many2one('grupo.report.micronizado','Grupo',required=True)
	...
	@api.one
	def get_monto(self):
		m = str(self.rm_report_micronizado_id.period_actual.code).split('/')
		m = int(m[0])
		mont = 0
		cant = {
			1: self.enero,
		...
		mont = cant[int(self.rm_report_micronizado_id.period_actual.code.split('/')[0])]
		self.monto = mont
	...
	def get_acumulado_pciento(self):
		if self.acumulado != 0:
			self.acumulado_pciento = self.acumulado / self.rm_report_micronizado_id.total_general
		else:
	...
	def get_promedio_pciento(self):
		if self.acumulado != 0:
			self.promedio_pciento = self.promedio / self.rm_report_micronizado_id.total_promedio_general
		else:
...
class rm_report_micronizado(models.Model):
	_name= 'rm.report.micronizado'

	fiscal = fields.Many2one('account.fiscalyear','Año Fiscal',required=True)
	sitio = fields.Char('Sitio',required=False)
	centro_de_costo = fields.Char('Centro de Costo',readonly=True,default='Micronizado')
	...
	conf_line_ids = fields.One2many('rm.report.micronizado.line','rm_report_micronizado_id','Lineas de Configuración')
	...
	def unlink(self):
		...
		return super(rm_report_micronizado,self).unlink()

	@api.one
	def copy(self,default):
		t= super(rm_report_micronizado,self).copy(default)
		for i in self.conf_line_ids:
			vals_i = {
				'rm_report_micronizado_id':t.id,
				'cuenta':i.cuenta,
				'tipo':i.tipo.id,
				'concepto':i.concepto,
				'grupo':i.grupo.id,
			...
			self.env['rm.report.micronizado.line'].create(vals_i)
		return t
	...
	def create(self,vals):
		...
		return super(rm_report_micronizado,self).create(vals)
	...
	def calculate(self):
		...
		self.env.cr.execute("""
update rm_report_micronizado_line opt set
enero= 0, febrero = 0,
marzo= 0, abril= 0,
mayo= 0, junio= 0,
julio= 0, agosto= 0,
...
where opt.rm_report_micronizado_id = """ + str(self.id) + """
""")
...
where opt.cuenta = saldos.nivel1 and opt.rm_report_micronizado_id = """ + str(self.id) + """
			""")
	...
	def get_pie_pagina(self):
		...
		if len(cp_obj) >0:
			...
			rpt.append([ cp_obj.micro_pro_ton, cp_obj.micro_pro_cp , cp_obj.micro_pro_imp , 0,0,0])
			rpt.append([ cp_obj.micro_ini_ton, cp_obj.micro_ini_cp , cp_obj.micro_ini_imp , 0,0,0])
			rpt.append([0,0,0,0,0,0])
			rpt.append([ cp_obj.micro_dis_ton, cp_obj.micro_dis_cp , cp_obj.micro_dis_imp , 0,0,0])
			rpt.append([0,0,0,0,0,0])
			rpt.append([0,0,0,0,0,0])
			rpt.append([0,0,0,0,0,0])
			rpt.append([ cp_obj.micro_ven_ton, cp_obj.micro_ven_cp , cp_obj.micro_ven_imp , 0,0,0])
			rpt.append([0,0,0,0,0,0])
			rpt.append([0,0,0,0,0,0])
			rpt.append([ cp_obj.micro_final_ton, cp_obj.micro_final_cp , cp_obj.micro_final_imp , 0,0,0])
		...
		for i in period_list:
			...
			if len(cp_obj) >0:
				...
				rpt[0][3] += cp_obj.calci_tt_ton
				rpt[0][5] += cp_obj.calci_tt_imp
				rpt[0][4] = 0 if rpt[0][3] == 0 else (rpt[0][5] / rpt[0][3] )

				rpt[1][3] += cp_obj.micro_pro_ton
				rpt[1][5] += cp_obj.micro_pro_imp
				rpt[1][4] = 0 if rpt[1][3] == 0 else (rpt[1][5] / rpt[1][3] )


				if i.code.split('/')[0] == '01':
					rpt[2][3] += cp_obj.micro_ini_ton
					rpt[2][5] += cp_obj.micro_ini_imp
				...
				rpt[8][3] += cp_obj.micro_ven_ton
				rpt[8][5] += cp_obj.micro_ven_imp
				...
				self.env.cr.execute(""" 
				   select sum(salida) as ingreso,sum(round(credit,2)) as debit from get_kardex_v("""+str(self.period_actual.date_start)[:4]+'0101'+""","""+str(self.period_actual.date_stop).replace('-','')+""",'{""" + str(parametros.pproduct_costos_micronizado.id) + """}',
				   '{""" + str(parametros.location_existencias_calcinacion.id) + """,""" + str(parametros.location_perdidas_mermas.id) + """}') 
				   where ((ubicacion_origen = """ + str(parametros.location_micro_perdidas_mermas.id) + """ and ubicacion_destino = """ + str(parametros.location_existencias_micronizado.id) + """)
				   or (ubicacion_destino = """ + str(parametros.location_micro_perdidas_mermas.id) + """ and ubicacion_origen = """ + str(parametros.location_existencias_micronizado.id) + """))
				...
				rpt[10][0] = tonex
				rpt[10][2] = preciox
				rpt[10][1] = 0 if rpt[10][3] == 0 else (rpt[10][2] / rpt[10][0] )
				...
				rpt[11][0] = rpt[11][0] - rpt[10][0]
				rpt[11][2] = rpt[11][2] - rpt[10][2]
				rpt[11][1] = 0 if rpt[11][0] == 0 else (rpt[11][2] / rpt[11][0] )




				rpt[11][3] = rpt[4][3] - rpt[8][3]
				rpt[11][5] = rpt[4][5] - rpt[8][5]
	...
	def get_valores(self):
		...
		for i in periodos:
			...
			if len(cp_obj) >0:
				cp_obj = cp_obj[0]		
				#### la primera linea
				rpt.append( cp_obj.micro_pro_ton )
	...
	def export_excel(self):
		...
		workbook = Workbook(direccion +'Reporte_Micronizado.xlsx')
		...
		f = open(direccion + 'Reporte_Micronizado.xlsx', 'rb')
		
		vals = {
			'output_name': 'Reportes_Mexicanos_Micronizado.xlsx',
