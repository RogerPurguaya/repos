...
class grupo_report_costos(models.Model):
	_name = 'grupo.report.costos'
...
class tipo_report_costos(models.Model):
	_name = 'tipo.report.costos'
...
class rm_report_costos_line(models.Model):
	_name = 'rm.report.costos.line'


	rm_report_costos_id = fields.Many2one('rm.report.costos','Cabezera')

		
	codigo_rel = fields.Char(u'Código')
	cuenta = fields.Char('Cuenta',required=False)
	tipo = fields.Many2one('tipo.report.costos','Tipo',required=True)
	concepto = fields.Char('Concepto',required=True)
	grupo = fields.Many2one('grupo.report.costos','Grupo',required=True)
...
class rm_report_costos(models.Model):
	_name= 'rm.report.costos'

	fiscal = fields.Many2one('account.fiscalyear','Año Fiscal',required=True)
	sitio = fields.Char('Sitio',required=False)
	centro_de_costo = fields.Char('Centro de Costo',readonly=True,default='Costos')
	...
	conf_line_ids = fields.One2many('rm.report.costos.line','rm_report_costos_id','Lineas de Configuración')
	...
	def unlink(self):
		...
		return super(rm_report_costos,self).unlink()

	@api.one
	def copy(self,default):
		t= super(rm_report_costos,self).copy(default)
		for i in self.conf_line_ids:
			vals_i = {
				'rm_report_costos_id':t.id,
				'cuenta':i.cuenta,
				'tipo':i.tipo.id,
				'concepto':i.concepto,
				'grupo':i.grupo.id,
			...
			self.env['rm.report.costos.line'].create(vals_i)
		return t
	...
	def create(self,vals):
		...
		return super(rm_report_costos,self).create(vals)
	...
	def calculate(self):
		...
		self.env.cr.execute("""
update rm_report_costos_line opt set
mes_monto= 0, mes_monto_dolares = 0
where opt.rm_report_costos_id = """ + str(self.id) + """
""")

		extra = self.env['rm.report.extraccion'].search( [('period_actual','=',self.period_actual.id)] )
		tritu = self.env['rm.report.trituracion'].search( [('period_actual','=',self.period_actual.id)] )
		calci = self.env['rm.report.calcinacion'].search( [('period_actual','=',self.period_actual.id)] )
		micro = self.env['rm.report.micronizado'].search( [('period_actual','=',self.period_actual.id)] )

		mes = self.period_actual.code.split('/')[0]
		dic_mes = {
		'00': 'enero',
		'01': 'enero',
		'02': 'febrero',
		...
		for i in self.conf_line_ids:
			tmp_re = 0
			if extra.id:
				for j in self.env['rm.report.extraccion.line'].search( [('rm_report_extraccion_id','=',extra.id),('codigo_rel','=',i.codigo_rel),('codigo_rel','!=',''),('codigo_rel','!=',False)] ):
					exec("tmp_re += j." + dic_mes[mes])

			if tritu.id:
				for j in self.env['rm.report.trituracion.line'].search( [('rm_report_trituracion_id','=',extra.id),('codigo_rel','=',i.codigo_rel),('codigo_rel','!=',''),('codigo_rel','!=',False)] ):
					exec("tmp_re += j." + dic_mes[mes])

			if calci.id:
				for j in self.env['rm.report.calcinacion.line'].search( [('rm_report_calcinacion_id','=',extra.id),('codigo_rel','=',i.codigo_rel),('codigo_rel','!=',''),('codigo_rel','!=',False)] ):
					exec("tmp_re += j." + dic_mes[mes])

			if micro.id:
				for j in self.env['rm.report.micronizado.line'].search( [('rm_report_micronizado_id','=',extra.id),('codigo_rel','=',i.codigo_rel),('codigo_rel','!=',''),('codigo_rel','!=',False)] ):
					exec("tmp_re += j." + dic_mes[mes])
			
			i.mes_monto = tmp_re
			i.mes_monto_dolares = ( 0 if self.tipo_cambio==0 else tmp_re / self.tipo_cambio)
	...
	def cabezera(self,c,wReal,hReal):
		...
		c.drawString( 404 + adicional, pos_inicial, "ANALÍTICO DE COSTOS DE OPERACIÓN")
	...
	def reporteador(self):
		...
		elements = self.env['rm.report.costos.line'].search([('rm_report_costos_id','=',self.id)]).sorted(key=lambda r: dig_5(r.tipo.order)+dig_5(r.grupo.order))
		flag = True
		n_grupo = None
		n_tipo = None
		ultimo_elem = None
	...
	def export_excel(self):
		...
		for i in elements:
			...
			if n_tipo != i.tipo:
				...
				worksheet.write(x,col, ((self.mes_monto)), numberdos)
				col += 1
				worksheet.write(x,col, ((self.mes_monto_dolares)), numberdos)
				col += 1
		...
		vals = {
			'output_name': 'Reportes_Mexicanos_Costos.xlsx',
