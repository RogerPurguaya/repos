...
class create_detraccion(models.Model):
	...
	def generar(self):
		...
		if invoice.name_move_detraccion and invoice.diario_move_detraccion.id == m.diario_detracciones.id and invoice.periodo_move_detraccion.id == invoice.period_id.id:
			data['name']= invoice.name_move_detraccion
			flag_ver = False
		else:
			invoice.diario_move_detraccion= m.diario_detracciones.id
			invoice.periodo_move_detraccion = invoice.period_id.id
			flag_ver = True
		lines = []
...
class account_invoice(models.Model):
	...
	periodo_move_detraccion = fields.Many2one('account.period','Periodo')
	...
	def action_cancel(self):
		...
		vals_data = {}
		ids_conciliar = []
		for i1 in self.move_detraccion_id.line_id:
			if i1.debit >0:
				ids_conciliar.append(i1.id)
		"""
	...
	def create_detraccion_gastos(self):
		...
		context = {'invoice_id': self.id,'default_fecha': self.date_invoice ,'default_monto':self.amount_total * float(self.partner_id.porcentaje)/100.0}
		return {
