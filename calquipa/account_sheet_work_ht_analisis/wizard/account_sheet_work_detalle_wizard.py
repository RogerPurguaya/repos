...
class account_sheet_work_analisis_ht_wizard(osv.TransientModel):
	...
	def do_rebuild(self):
		...
		CREATE OR REPLACE view account_sheet_work_analisis_ht as (
			...
			select  
			1000001 as id,
			''::varchar as resultado_mxn,
			''::varchar as balance_mxn,
		...
		if self.type_show == 'pantalla':
			...
			worksheet.write(3,0, "Resultado MXN",boldbord)
			worksheet.write(3,1, "Balance MXN",boldbord)
			worksheet.write(3,2, "EE FF PERU",boldbord)
			worksheet.write(3,3, "Nivel 1",boldbord)
			worksheet.write(3,4, "Nivel 2",boldbord)
			worksheet.write(3,5, "Nivel 3",boldbord)
			worksheet.write(3,6, "Nivel 4",boldbord)
			worksheet.write(3,7, "Cuenta",boldbord)
			worksheet.write(3,8, u"Descripción",boldbord)
			worksheet.write(3,9, "Debe",boldbord)
			worksheet.write(3,10, "Haber",boldbord)
			worksheet.write(3,11, u"Saldo Deudo",boldbord)
			worksheet.write(3,12, "Saldo Acreedor",boldbord)
			worksheet.write(3,13, "Activo",boldbord)
			worksheet.write(3,14, "Pasivo",boldbord)
			worksheet.write(3,15, "Perdidas Nat.",boldbord)
			worksheet.write(3,16, "Ganancia Nat.",boldbord)
			worksheet.write(3,17, "Perdidas Fun.",boldbord)
			worksheet.write(3,18, u"Ganancia Fun.",boldbord)

			for i in self.env['account.sheet.work.analisis.ht'].search(filtro):
				worksheet.write(x,0, i.resultado_mxn if i.resultado_mxn else '',bord)
				worksheet.write(x,1, i.balance_mxn if i.balance_mxn else '',bord)
				worksheet.write(x,2, i.ee_ff_peru if i.ee_ff_peru else '',bord)
				worksheet.write(x,3, i.nivel1 if i.nivel1 else '',bord)
				worksheet.write(x,4, i.nivel2 if i.nivel2 else '',bord)
				worksheet.write(x,5, i.nivel3 if i.nivel3 else '',bord)
				worksheet.write(x,6, i.nivel4 if i.nivel4 else '',bord)

				worksheet.write(x,7, i.cuenta if i.cuenta else '',bord)
				worksheet.write(x,8, i.descripcion ,bord)
				worksheet.write(x,9, i.debe ,numberdos)
				worksheet.write(x,10, i.haber ,numberdos)

				worksheet.write(x,11, i.saldodeudor ,numberdos)
				worksheet.write(x,12, i.saldoacredor ,numberdos)
				worksheet.write(x,13, i.activo ,numberdos)
				worksheet.write(x,14, i.pasivo ,numberdos)
				worksheet.write(x,15, i.perdidasnat ,numberdos)
				worksheet.write(x,16, i.ganancianat ,numberdos)
				worksheet.write(x,17, i.perdidasfun ,numberdos)
				worksheet.write(x,18, i.gananciafun ,numberdos)
