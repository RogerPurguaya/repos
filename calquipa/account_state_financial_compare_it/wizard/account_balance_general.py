...
full join  get_balance_general(""" + flag+ """ ,periodo_num('""" + self.periodo_ini_c.name+"""') ,periodo_num('""" +self.periodo_fin_c.name +"""' )) AS A2 on (A1.name= A2.name and A1.grupo = A2.grupo)
	...
	order by orden,name """)
			...
			worksheet.write(x,5, 'PASIVO Y PATRIMONIO' , bold)
			worksheet.write(x,7, self.fiscalyear_id.name + '(' + self.periodo_ini.code[:2] +' - ' + self.periodo_fin.code[:2] + ')' , bold)
			worksheet.write(x,6, self.fiscalyear_c_id.name + '(' + self.periodo_ini_c.code[:2] +' - ' + self.periodo_fin_c.code[:2] + ')' , bold)
...
order by orden,name """)
		...
		c.drawString( (wReal)-140 , pos_inicialR, self.fiscalyear_id.name + '(' + self.periodo_ini.code[:2] +' - ' + self.periodo_fin.code[:2] + ')' )
		c.drawString( (wReal)-75 , pos_inicialR, self.fiscalyear_c_id.name + '(' + self.periodo_ini_c.code[:2] +' - ' + self.periodo_fin_c.code[:2] + ')' )
