...
base1,base2,base3,cng,igv1,igv2,igv3,otros,isc,total from (SELECT move_id,
 SUM(BASE1) AS BASE1,
 SUM(BASE2) AS BASE2, 
 SUM(BASE3) AS BASE3,
 SUM(CNG) AS CNG, 
...
order by periodo,libro,voucher
		...
		if self.type_show == 'excel':
			...
			worksheet.write(4,4, u"RUC",boldbord)
			worksheet.write(4,5, "T.D.P.",boldbord)
			worksheet.write(4,6, "Empresa",boldbord)
			worksheet.write(4,7, u"T.C.",boldbord)
			worksheet.write(4,8, u"Nro Comprobante",boldbord)
			...
			for line in self.env['account.purchase.register.nodeducible'].search([]):
				...
				worksheet.write(x,4,line.type_number if line.type_number else '',bord)
				worksheet.write(x,5,line.tdp if line.tdp else '',bord)
				worksheet.write(x,6,line.empresa if line.empresa else '',bord)
				worksheet.write(x,7,line.tc if line.tc else '',bord)
				worksheet.write(x,8,line.nro_comprobante if line.nro_comprobante  else '',bord)
