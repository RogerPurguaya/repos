...
left join (select concat(partner_id,account_id,type_document_id,supplier_invoice_number) as identifica,date,date_due from account_invoice where state<>'cancel' and state<>'draft' ) facturas on facturas.identifica=t.identifica
order by empresa, code, nro_comprobante
		...
		if self.mostrar == 'excel':
			...
			worksheet.write(9,3, "Tipo Cuenta",boldbord)
			worksheet.write(9,4, u"Cuenta",boldbord)
			worksheet.write(9,5, "Tipo Documento",boldbord)
			worksheet.write(9,6, "Nro. Comprobante",boldbord)
			worksheet.write(9,7, "Debe",boldbord)
			worksheet.write(9,8, "Haber",boldbord)
			worksheet.write(9,9, "Saldo",boldbord)
			worksheet.write(9,10, "Divisa",boldbord)
