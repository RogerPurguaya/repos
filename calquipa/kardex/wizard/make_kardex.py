 ...
 , CASE WHEN A2.name_template is not null THEN A2.name_template ELSE A1."Producto" END as name_template, coalesce(A1.debe,0) as montokardex, coalesce(A2.debit,0) as monto, coalesce(A1.debe,0) - coalesce(A2.debit,0) as diference    from (
	...
	def do_csvtoexcel(self):
		...
		if self.env.context['tipo']=='valorado':
			...
			worksheet.merge_range(8,14,9,14, u"Costo Adquisición",boldbord)
			worksheet.merge_range(8,15,9,15, "Costo Promedio",boldbord)

			worksheet.merge_range(8,16,9,16, "Ubicacion Origen",boldbord)
			worksheet.merge_range(8,17,9,17, "Ubicacion Destino",boldbord)
			worksheet.merge_range(8,18,9,18, "Almacen",boldbord)
			...
			for line in self.env.cr.fetchall():
				...
				worksheet.write(x,14,line[14] if line[14] else 0 ,numberseis )
				worksheet.write(x,15,line[15] if line[15] else 0 ,numberocho )
				...
				ingreso1 += line[11] if line[11] else 0
				salida1 += line[12] if line[12] else 0
			...
			worksheet.write(x,12,salida1 ,numberdosbold )
	...
	def do_csv(self,cr,uid,ids,context=None):
		...
		if context['tipo']=='valorado':
			cadf="""
				...
				pedido_venta as "Pedido de Venta",
