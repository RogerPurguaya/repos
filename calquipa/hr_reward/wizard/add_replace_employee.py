				...
				if reward.period == '07':
					...
					for employee in employees:
						...
						if  (not employee.fecha_cese) or (datetime.strptime(str(employee.fecha_cese), '%Y-%m-%d').year == int(self.year.code) and datetime.strptime(str(employee.fecha_cese), '%Y-%m-%d').month > 6):
							...
							if months >= 1:
								hr_param = self.env['hr.parameters'].search([('num_tipo','=',10001)])[0]	
								a_familiar = 0
								if employee.children_number > 0:
									a_familiar = hr_param.monto
								he_night = []
								ex_plus = 0 
								absences = 0
								for tareo in tareos:
									...
									if periodo[1] == reward.year.code and periodo[0] in ['01', '02', '03', '04', '05', '06']:
										#Cálculo de faltas
										absences += self.env['hr.tareo.line'].search([('tareo_id','=',tareo.id),('employee_id','=',employee.id)]).dias_suspension_perfecta
