...
class Show_Detail_Tracing_Line_Wizard(models.TransientModel):
	...
	def _get_invoice(self):
		for rec in self:
			if rec.lot_line_id.order_prod_id.invoice_ids:
				invoice = rec.lot_line_id.order_prod_id.invoice_ids[0]
				rec.invoice = invoice.id
				rec.invoice_number = invoice.number if invoice.number else 'Factura pendiente de validacion'
	
	@api.depends('lot_line_id')
	def _get_required(self):
		for rec in self:
			sale_lines = rec.order_id.sale_lines.filtered(lambda x: x.product_id.id == rec.lot_line_id.product_id.id)
			count_required = lot_line_id.calc_line_id
			if len(sale_lines) > 0:
	...
	def _get_produced(self):
		for rec in self:
			lot_lines = rec.order_id.line_ids.mapped('lot_line_id').filtered(lambda x: x.product_id.id == rec.lot_line_id.product_id.id)
			produced = lot_lines.filtered(lambda x: x.templado)
			if len(produced) > 0:
				rec.area_produced = reduce(lambda x,y: x+y,produced.mapped('area'))
