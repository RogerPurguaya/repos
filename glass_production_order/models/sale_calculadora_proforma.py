...
class SaleCalculatorProformaLine(models.Model):
	...
	def default_get(self,fields):
		res = super(SaleCalculatorProformaLine,self).default_get(fields)
		try:
		...
		if conf.default_pulido:
			res.update({'pulido1': conf.default_pulido.id})
		return res
	...
	def unlink(self):
		for line in self:
			if line.production_id:
				raise UserError(u"No se puede eliminar una línea que tiene asignada una orden de producción")			
