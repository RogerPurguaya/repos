...
# 	_inherit = 'account.cashbox.line'
	...
	#default_pos_id = fields.Many2one('cashbox.config', string='This cashbox line is used by default when opening or closing a balance for this point of sale')

	# default_cashbox_id = fields.Many2one('cashbox.config', string='This cashbox line is used by default when opening or closing a balance for this point of sale')
...
class CashboxConfig(models.Model):
	_name = 'cashbox.config'

	# Creaar un diario exclusivamente par nosotros ??'
	def _default_sale_journal(self):
		journal = self.env.ref('cashbox.pos_sale_journal', raise_if_not_found=False)
	...
	def _get_group_cashbox_manager(self):
		return self.env.ref('cashbox.group_cashbox_manager')

	def _get_group_cashbox_user(self):
		return self.env.ref('cashbox.group_cashbox_manager')
	...
	session_ids = fields.One2many('cashbox.session', 'config_id', string='Sessions')
	current_session_id = fields.Many2one('cashbox.session', compute='_compute_current_session', string="Current Session")
	current_session_state = fields.Char(compute='_compute_current_session')
	last_session_closing_cash = fields.Float(compute='_compute_last_session')
	last_session_closing_date = fields.Date(compute='_compute_last_session')
	cashbox_session_username = fields.Char(compute='_compute_current_session_user')
	...
	group_cashbox_manager_id = fields.Many2one('res.groups', string='Point of Sale Manager Group', default=_get_group_cashbox_manager,
		help='This field is there to pass the id of the pos manager group to the point of sale client')
	group_cashbox_user_id = fields.Many2one('res.groups', string='Point of Sale User Group', default=_get_group_cashbox_user,
	...
	def _compute_currency(self):
		for cashbox_config in self:
			if cashbox_config.journal_id:
				cashbox_config.currency_id = cashbox_config.journal_id.currency_id.id or cashbox_config.journal_id.company_id.currency_id.id
			else:
				cashbox_config.currency_id = self.env.user.company_id.currency_id.id

	@api.depends('session_ids')
	def _compute_current_session(self):
		for cashbox_config in self:
			session = cashbox_config.session_ids.filtered(lambda r: r.user_id.id == self.env.uid and \
				not r.state == 'closed' and \
				'(RESCUE FOR' not in r.name)
			# sessions ordered by id desc
			cashbox_config.current_session_id = session and session[0].id or False
			cashbox_config.current_session_state = session and session[0].state or False

	@api.depends('session_ids')
	def _compute_last_session(self):
		CashboxSession = self.env['cashbox.session']
		for cashbox_config in self:
			session = CashboxSession.search_read(
				[('config_id', '=', cashbox_config.id), ('state', '=', 'closed')],
				['cash_register_balance_end_real', 'stop_at'],
				order="stop_at desc", limit=1)
			if session:
				cashbox_config.last_session_closing_cash = session[0]['cash_register_balance_end_real']
				cashbox_config.last_session_closing_date = session[0]['stop_at']
			else:
				cashbox_config.last_session_closing_cash = 0
				cashbox_config.last_session_closing_date = False

	@api.depends('session_ids')
	def _compute_current_session_user(self):
		for cashbox_config in self:
			session = cashbox_config.session_ids.filtered(lambda s: s.state == 'opened' and '(RESCUE FOR' not in s.name)
			cashbox_config.cashbox_session_username = session and session[0].user_id.name or False
	...
	def name_get(self):
		result = []
		for config in self:
			last_session = self.env['cashbox.session'].search([('config_id', '=', config.id)], limit=1)
			if (not last_session) or (last_session.state == 'closed'):
				result.append((config.id, config.name + ' (' + _('no usado') + ')'))
				continue
	...
	def open_session_cb(self):
		...
		if not self.current_session_id:
			self.current_session_id = self.env['cashbox.session'].create({
