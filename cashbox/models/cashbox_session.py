...
class CashboxSession(models.Model):
	...
	def _compute_cash_all(self):
		for session in self:
			session.cash_journal_id = session.cash_register_id = session.cash_control = False
			session.cash_control = bool(session.config_id.cash_control)
			#if session.config_id.cash_control:
			#	session.cash_control = True
				# for statement in session.statement_ids:
				# 	if statement.journal_id.type == 'cash':
	...
	def create(self, values):
		...
		pos_config = self.env['cashbox.config'].browse(config_id)
		ctx = dict(self.env.context, company_id=pos_config.company_id.id)
		if not pos_config.journal_id:
			# si no tiene diario, raise en prima :v
	...
	# 	return super(CashboxSession, self).unlink()
	...
	def action_pos_session_close(self):
		...
		return {
			...
			'name': 'Caja Menu',
			'tag': 'reload',
			'params': {'menu_id': self.env.ref('cashbox.menu_cashbox_root').id},
		}
