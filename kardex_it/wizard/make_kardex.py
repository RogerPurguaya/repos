...
class make_kardex(models.TransientModel):
	...
	def do_csvtoexcel(self):
		...
		if self.env.context['tipo']=='valorado':
			...
			worksheet.merge_range(8,3,9,3, u"Partner",boldbord)
			worksheet.merge_range(8,4,9,4, u"Tipo de Operación",boldbord)
			worksheet.merge_range(8,5,9,5, u"Categoria",boldbord)

			worksheet.merge_range(8,6,9,6, u"Producto",boldbord)
			worksheet.merge_range(8,7,9,7, u"Codigo P.",boldbord)
			worksheet.merge_range(8,8,9,8, u"Unidad",boldbord)
...
almacen.id,pp.id,vstf.fecha,vstf.entrada desc
			...
			for line in self.env.cr.fetchall():
				if almacen == None:
					...
					producto = (line[6] if line[6] else '')
					saldo = line[11] - line[12]
				elif almacen != (line[2] if line[2] else '') or producto != (line[6] if line[6] else ''):
					almacen = (line[2] if line[2] else '')
					producto = (line[6] if line[6] else '')
					saldo = line[11] - line[12]
				else:
			...
			tam_col = [11,11,20,12,11,7,5,11,11,11,11,11,11,11,11,11,11,11,11,11,11,11,11,11]
